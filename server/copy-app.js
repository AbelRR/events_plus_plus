...
app.get('/lastOrder/:_id', (req, res) => {
  ...
  clients.selectById(_id, (err, data) => {
    if (err) {
      ...
      const {
        name, address, phone, openOrders,
      } = data[0];

      const {
        notes, balanceOwed, contact, from, to, event, orderLocation, delivered, pickedUp,
      } = lastOrder;
...
app.post('/orders', (req, res) => {
  ...
  const {
    contact, notes, balanceOwed, orderLocation, delivered, pickedUp,
  } = orderDetails;
